...
IOU_SUCCESS_THRES = 0.85 # 현재 물체와 목표 물체의 겹치는 면적 비율(IoU)이 96% 이상이면 성공
...
T_ALLOWED_STROKES_MID = [0.5, 0.8]
T_ALLOWED_STROKES_NEAR = [0.2, 0.4]
...
SQUARE_ALLOWED_STROKES_FAR = [1.2, 1.6]
SQUARE_ALLOWED_STROKES_MID = [0.7, 0.9]
SQUARE_ALLOWED_STROKES_NEAR = [0.2, 0.5]

CIRCLE_ALLOWED_STROKES_FAR = [1.2, 1.6]
CIRCLE_ALLOWED_STROKES_MID = [0.7, 0.9]
CIRCLE_ALLOWED_STROKES_NEAR = [0.2, 0.5]
...
STROKE_DIST_FAR = 0.1 / REAL_WORKSPACE_SIZE_X 
...
# 이를 전체 공간 크기(15cm)로 나누어 0.0~1.0 사이의 비율(정규화)로 변환해 저장합니다.
#MAX_STROKE_LEN = 0.02 / REAL_WORKSPACE_SIZE_X
...
IOU_EARLY_SUCCESS_THRES = 0.93 # 93% 정도면 '거의 성공(Early Success)'으로 보고 세밀한 조작 시작
SHAPE_ALIGN_EARLY_SUCCESS_THRES = SHAPE_ALIGN_SUCCESS_THRES * 1.25 # 약간 느슨한 오차 기준
...
# =========================================================
# 6. Planner scoring (구버전 또는 다른 플래너 모드용 점수 가중치 - 현재는 거의 안 씀)
# =========================================================
PLANNER_PROGRESS_WEIGHT = 10.0
PLANNER_ALIGN_WEIGHT = 1.8
PLANNER_IOU_WEIGHT = 0.0
PLANNER_PATH_WEIGHT = 0.10
PLANNER_ROT_WEIGHT = 0.03
PLANNER_MISALIGN_PENALTY = 0.35
PLANNER_FINAL_IOU_GATE = 0.90
PLANNER_FINAL_PROGRESS_BONUS = 0.60
...
#DIRECT_ACTION_TYPES = ("normal", "spin_pos", "spin_neg")
DIRECT_ACTION_TYPES = ("normal")
# 휴리스틱 모드에서 스핀 먹일 때 접선 방향 스케일
#DIRECT_SPIN_TANGENT_SCALE = 0.5
DIRECT_SPIN_TANGENT_SCALE = 0.25
# 목표에 거의 다 왔을 때 힘을 원래의 30%로 줄임
#DIRECT_NEAR_GOAL_STROKE_SCALE = 0.30
# 아무리 짧게 밀어도 로봇 푸셔 반지름의 75%보다는 길게 밀어야 함 (찔끔거림 방지)
#DIRECT_MIN_STROKE_LEN = 0.5 * PUSHER_RADIUS
...
DIRECT_NEAR_GOAL_THRESH = 0.10 / REAL_WORKSPACE_SIZE_X
...
DIRECT_ACTION_SHORTLIST_TOPK = 10
