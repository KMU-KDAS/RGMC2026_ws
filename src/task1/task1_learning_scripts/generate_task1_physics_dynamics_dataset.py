...
예시
python generate_task1_physics_dynamics_dataset.py --output data/task1_physics_dynamics.jsonl --num-records 50000
"""
...
try:
    ...
    from task1.geometry_utils import get_global_corners
except Exception:
    import config  # type: ignore
    from brain import PushingBrain  # type: ignore
    from geometry_utils import get_global_corners  # type: ignore
...
def pose_to_polygon(shape_type: str, pose: List[float]):
    if shape_type == "WEIGHTED_CIRCLE":
        return None
    return np.asarray(get_global_corners(pose, config.get_shape_corners(shape_type)), dtype=float)
...
def main() -> None:
    ...
    parser.add_argument("--shape-types", type=str, default="T_BASE,WEIGHTED_SQUARE,WEIGHTED_CIRCLE")
    ...
    shape_types = tuple(s.strip() for s in args.shape_types.split(",") if s.strip())
    ...
    with out_path.open("w", encoding="utf-8") as f:
        while num_written < args.num_records:
            num_pose_trials += 1
            ...
            shape_info = (
                ...
                if shape_type == "WEIGHTED_CIRCLE"
                else np.asarray(get_global_corners(current_pose, local_corners), dtype=float)
            )
            ...
            face_candidates = brain.generate_face_candidates(shape_type, shape_info, radius, base_stroke)
            if not face_candidates:
                continue

            # generate_candidate_actions는 filtered_faces 형식을 받으므로 간단히 감싸준다.
            filtered_faces = [{"face_info": c} for c in face_candidates]
            global_err = 0.0
            try:
                from task1.geometry_utils import shape_alignment_error
            except Exception:
                from geometry_utils import shape_alignment_error  # type: ignore
            global_err = shape_alignment_error(shape_type, current_pose, target_pose, local_corners, radius)
            ...
            candidates = choose_random_candidates(candidates, args.max_candidates_per_pose, rng)
            belief_name = brain.com_belief[shape_type]

            for cand in candidates:
                if num_written >= args.num_records:
                    break

                physics_next = brain._simulate_case(shape_type, current_pose, cand, belief_name)
                ...
                if args.progress_every > 0 and num_written % args.progress_every == 0:
                    print(f"[GEN] wrote {num_written}/{args.num_records} records | pose_trials={num_pose_trials}", flush=True)

    print(f"[DONE] output: {out_path}")
    print(f"[DONE] records: {num_written} | pose_trials={num_pose_trials}")
